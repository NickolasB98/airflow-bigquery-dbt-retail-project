...
This phase uses Airflow  to automate the process of fetching data from a source.
Airflow retrieves a CSV file containing retail sales data from a public dataset on Kaggle.
The script defines the location of the data, uploads it to Google Cloud Storage (GCS), and specifies the target BigQuery dataset (retail) where the data will be loaded.

**2. Data Transformation (dbt)**

dbt, a data transformation tool, is used to clean, transform, and structure the raw data for analysis.
The project defines several dbt models:

dim_customer: Creates a dimension table for customers, including a surrogate key and country information.

dim_datetime: Creates a dimension table for dates and time components extracted from the invoice date.

dim_product: Creates a dimension table for products, including a surrogate key, description, and unit price. Handles potential duplicates based on StockCode having different prices.

fct_invoices: Creates a fact table for invoices, containing foreign keys to the dimension tables and calculated fields like total amount.

dbt models ensure data quality by enforcing data types, checking for missing values, and identifying duplicates.

**3. Data Validation (Soda)**

Soda, a data validation tool, verifies the quality and consistency of the transformed data.

The project defines checks for each dbt model table, ensuring they have the expected schema, no missing values in crucial columns, and follow specific data integrity rules (e.g., unique identifiers, valid date ranges).

Soda helps identify potential issues before relying on the data for analysis.

**4. Data Modeling & Reporting (dbt)**

dbt is used again to create reports based on the transformed data.

Three reports are created:

report_customer_invoices: Analyzes total invoices and revenue by customer country.

report_product_invoices: Analyzes total quantity sold by product.

report_year_invoices: Analyzes the number of invoices and total revenue per month.

**5. Data Visualization (Metabase)**

Metabase, a business intelligence tool, is used to create visualizations from the reports generated by dbt.

The project sets up a Metabase container and configures it to connect to the BigQuery dataset containing the reports.

Sample visualizations are suggested:

Top 10 Products by Quantity Sold (Pie Chart)
